...
try:
    ...
    import pandas as pd
    tz = _secret_or_env("timezone")
    cur = _secret_or_env("base_currency")
    rsd = _secret_or_env("report_start_date")
    ads = _secret_or_env("ads_enabled")
    snap = _secret_or_env("auto_snapshot_pdf")
    fallback_rows = [
        {"key": "timezone", "value": _fmt(tz)},
        {"key": "base_currency", "value": _fmt(cur)},
        {"key": "report_start_date", "value": _fmt(rsd)},
        {"key": "ads_enabled", "value": _fmt(ads)},
        {"key": "auto_snapshot_pdf", "value": _fmt(snap)},
    ]
    st.caption("Not connected to Google Sheets — showing values from secrets/env if present.")
