...
async def api_list_products(limit: int = Query(50, ge=1, le=500), q: Optional[str] = None, db: AsyncSession = Depends(get_db)):
    return await list_products(db, limit=limit, q=q)

@app.post("/v1/products", response_model=ProductOut)
async def api_create_product(body: ProductIn, db: AsyncSession = Depends(get_db), x_api_key: str | None = Header(default=None)):
...
async def debug_ads():
    ...
    return {"ok": status_camps == 200, "stage": "campaigns" if status_camps==200 else "campaigns_error", "status": status_camps, "body": body_camps}
