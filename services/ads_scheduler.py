...
from services.amazon_ads_service import _init_db, fetch_metrics, fetch_search_terms, fetch_placements

FREQ_MIN = int(os.getenv("SCHEDULE_FREQUENCY_MIN", "60"))
LOOKBACK_DAYS = int(os.getenv("ADS_LOOKBACK_DAYS", "30"))
AD_TYPES = [t.strip().upper() for t in os.getenv("ADS_TYPES", "SP,SB").split(",") if t.strip()]
...
def _run_once():
    ...
    for t in AD_TYPES:
        try:
            m = fetch_metrics(t, s, e) or []
            inserted["metrics"] += len(m)
        except Exception as ex:
            print(f"[scheduler] fetch_metrics {t} error: {ex}")
        try:
            srows = fetch_search_terms(t, s, e) or []
            inserted["search_terms"] += len(srows)
        except Exception as ex:
            print(f"[scheduler] fetch_search_terms {t} error: {ex}")
        try:
            prows = fetch_placements(t, s, e) or []
            inserted["placements"] += len(prows)
        except Exception as ex:
            print(f"[scheduler] fetch_placements {t} error: {ex}")
