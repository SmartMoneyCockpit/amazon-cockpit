...
if df.empty:
    st.info("No revenue protection alerts yet. Ensure `alerts_out_margins` and `alerts_out_ppc` exist (snapshot runner) and inventory has SKU/ASIN mapping.")
else:
    # KPI tiles
    ...
    st.write(f'{sheet_link("alerts_out_revenue_protection")} · {sheet_link("alerts_out_margins")} · {sheet_link("alerts_out_ppc")}', unsafe_allow_html=True)
    st.write("")
