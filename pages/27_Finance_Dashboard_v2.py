...
st.set_page_config(page_title="Finance Dashboard v2", layout="wide"); st.title("Finance Dashboard v2")
_last_pull_utc=None
def _read_finances():
    global _last_pull_utc
    try:
        sc=SheetsClient(); rows=sc.read_table("Finances"); _last_pull_utc=dt.datetime.utcnow(); return rows
    except Exception: _last_pull_utc=None; return []
rows=_read_finances(); kpis, df = build_kpis(rows)
st.subheader("Filters"); today=dt.date.today(); default_start=(today-dt.timedelta(days=30))
c1,c2,c3=st.columns([1.2,1.2,1])
with c1: start=st.date_input("Start date", value=default_start)
with c2: end=st.date_input("End date", value=today)
with c3: show_table=st.checkbox("Show raw table", value=False)
if not df.empty and "date" in df.columns:
    df["date"]=pd.to_datetime(df["date"], errors="coerce").dt.date; mask=(df["date"]>=start)&(df["date"]<=end); df_view=df.loc[mask].copy()
else: df_view=df.copy()
flags, df_annot=detect_anomalies(df_view, ["gmv","acos","tacos","refund_rate"], 14, 2.0)
def _pill(ok,label):
    color="#ef4444" if ok else "#16a34a"; text="Anomaly" if ok else "Normal"
...
if any(flags.values()):
    st.divider(); st.subheader("Actions")
    if st.button("Queue Anomalies → Digest"):
        try: path=append_anomalies_to_digest(flags, 2.0); st.success(f"Queued to {path}.")
...
fresh_text="—"; 
if _last_pull_utc is not None: fresh_text=_last_pull_utc.strftime("%Y-%m-%d %H:%M:%SZ UTC")
...
with k5: st.caption(f"Last updated: {fresh_text}")
st.divider(); st.subheader("Time Series"); st.caption("If Google Sheets is not connected, demo data is shown for preview.")
if not df_view.empty and "date" in df_view.columns and "gmv" in df_view.columns:
    st.area_chart(df_view.set_index("date")[["gmv"]])
else: st.info("No finance data available for the selected range.")
if not df_annot.empty:
    st.divider(); st.subheader("Anomaly Details (last 30 rows)")
    show_cols=[]; 
    for c in ["gmv","acos","tacos","refund_rate"]:
        if c in df_annot.columns: show_cols += [c, f"{c}_z", f"{c}_anomaly"]
    with st.expander("Show anomaly table", expanded=False):
        st.dataframe(df_annot.tail(30)[["date"]+show_cols] if "date" in df_annot.columns else df_annot.tail(30)[show_cols], use_container_width=True)
