...
import os
import io  # for XLSX export buffer
...
st.caption("Shows sessions, CVR, units, inventory & reviews. If your Google Sheet is empty, this page falls back to a small sample so it's never blank.")
...
if dfv.empty:
    st.info("No rows found. Use Catalog Enrichment and populate your Google Sheet tabs (`inventory` or `product_tracker`), or use the sample template below.")
else:
...
if doc_col and not dfv.empty:
    at_risk = int(pd.to_numeric(dfv[doc_col], errors="coerce").fillna(999).lt(10).sum())
    k4.metric("At Risk (<10 DoC)", f"{at_risk}")
else:
...
st.download_button("⬇️ Download inventory_sample.csv", data=template_csv, file_name="inventory_sample.csv", mime="text/csv")
